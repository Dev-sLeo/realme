          ...
          <?php if (!empty($lista)) : ?>
            <ul class="o-unifique__list" role="list">
              <?php foreach ($lista as $row) :
                ...
                $pct  = isset($row['porcentagem']) ? (int) $row['porcentagem'] : 0;
                $ltxt = isset($row['texto']) ? (string) $row['texto'] : '';

                if ($pct === 0 && $ltxt === '') continue;
              ?>
                <li class="o-unifique__list-item">
                  ...
                  <span class="o-unifique__list-icon" aria-hidden="true"><svg width="8" height="14" viewBox="0 0 8 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                      <path d="M0.832031 0.833252L6.83203 6.83325L0.832031 12.8333" stroke="#4353FA" stroke-width="1.66667" stroke-linecap="round" stroke-linejoin="round" />
                    </svg>
                  </span>
          ...
          <?php if ($btn_has) : ?>
            <div class="o-unifique__actions">
              <a class="button button__blue"
                ...
                target="<?= esc_attr($btn_tgt); ?>" <?= $btn_rel ? 'rel="' . esc_attr($btn_rel) . '"' : ''; ?>>
                <?= esc_html($btn_lbl); ?>
              </a>
            </div>
          <?php endif; ?>
            ...
            ?>
              <article class="c-step o-unifique__card" role="listitem">
                <div class="c-step__top">
                  ...
                  <span class="c-step__arrow" aria-hidden="true"><svg width="12" height="21" viewBox="0 0 12 21" fill="none" xmlns="http://www.w3.org/2000/svg">
                      <path d="M1 20L10.5 10.5L1 1" stroke="#85C3D6" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                    </svg>
                  </span>
                </div>
