...
<section class="o-product-comparison">
  <div class="s-container">
    ...
    <div class="o-product-comparison__columns">
      <article class="o-product-comparison__column o-product-comparison__column--left">
        <?php if (!empty($left['title'])) : ?><h3><?php echo esc_html($left['title']); ?></h3><?php endif; ?>
        <?php if (!empty($left['items']) && is_array($left['items'])) : ?>
          <ul>
            <?php foreach ($left['items'] as $item) : if (empty($item['text'])) continue; ?>
              <li class="is-<?php echo esc_attr($item['state'] ?? 'neutral'); ?>"><?php echo esc_html($item['text']); ?></li>
            <?php endforeach; ?>
          </ul>
        <?php endif; ?>
      </article>

      <article class="o-product-comparison__column o-product-comparison__column--right">
        <?php if (!empty($right['title'])) : ?><h3><?php echo esc_html($right['title']); ?></h3><?php endif; ?>
        <?php if (!empty($right['items']) && is_array($right['items'])) : ?>
          <ul>
            <?php foreach ($right['items'] as $item) : if (empty($item['text'])) continue; ?>
              <li class="is-<?php echo esc_attr($item['state'] ?? 'neutral'); ?>"><?php echo esc_html($item['text']); ?></li>
            <?php endforeach; ?>
          </ul>
        <?php endif; ?>
