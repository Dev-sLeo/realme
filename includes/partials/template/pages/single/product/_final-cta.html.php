...
<section class="o-product-final-cta">
  ...
  <div class="s-container">
    <div class="o-product-final-cta__content">
      <div>
        <?php if (!empty($data['title'])) : ?><h2 class="title__normal" data-animate="fade-up" data-animate-delay="0.1"><?php echo esc_html($data['title']); ?></h2><?php endif; ?>
        <?php if (!empty($data['description'])) : ?><div class="text__normal" data-animate="fade-up" data-animate-delay="0.2"><?php echo wpautop(wp_kses_post($data['description'])); ?></div><?php endif; ?>
        <div class="o-product-final-cta__actions" data-animate="fade-up" data-animate-delay="0.3">
        ...
        <?php if (!empty($data['bullets']) && is_array($data['bullets'])) : ?>
          <ul class="o-product-final-cta__bullets">
            <?php foreach ($data['bullets'] as $i => $bullet) : if (empty($bullet['text'])) continue; ?><li data-animate="fade-up" data-animate-delay="<?php echo 0.4 + ($i * 0.1); ?>s"><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
      ...
      <?php if (!empty($data['art']['ID'])) : ?><div class="o-product-final-cta__image" data-animate="fade-up" data-animate-delay="0.5"><?php echo wp_get_attachment_image((int) $data['art']['ID'], 'large', false, array('loading' => 'lazy')); ?></div><?php endif; ?>
    </div>
  </div>
</section>