...
<section class="o-hero" aria-label="<?php echo esc_attr($title ?: 'Hero'); ?>">
  <div class="o-hero__container">

    <div class="o-hero__content">
      <?php if (!empty($badge)): ?>
        <p class="subtitulo o-hero__badge">
          <?php echo esc_html($badge); ?>
        </p>
      <?php endif; ?>

      <?php if (!empty($title)): ?>
        <h1 class="title__super o-hero__title">
          <?php echo esc_html($title); ?>
        </h1>
      <?php endif; ?>

      <?php if (!empty($description)): ?>
        <div class="text__normal o-hero__text">
          <?php echo wpautop(wp_kses_post($description)); ?>
        </div>
      <?php endif; ?>

      <?php if (!empty($cta_primary_url) || !empty($cta_secondary_url)): ?>
        <div class="o-hero__ctas" role="group" aria-label="Ações do hero">

          <?php if (!empty($cta_primary_url) && !empty($cta_primary_title)): ?>
            <a class="button button__primary o-hero__cta" href="<?php echo esc_url($cta_primary_url); ?>"
          ...
          <?php if (!empty($cta_secondary_url) && !empty($cta_secondary_title)): ?>
            <a class="button-border__primary o-hero__cta" href="<?php echo esc_url($cta_secondary_url); ?>"
              target="<?php echo esc_attr($cta_secondary_target); ?>"
              rel="<?php echo ($cta_secondary_target === '_blank') ? 'noopener noreferrer' : 'nofollow'; ?>">
              <?php echo esc_html($cta_secondary_title); ?>
            </a>
          <?php endif; ?>
      ...
      <?php if (!empty($metrics) && is_array($metrics)): ?>
        <ul class="o-hero__metrics" aria-label="Métricas">
          <?php foreach ($metrics as $m):
            ...
            if (empty($value) && empty($label) && empty($icon_id))
              continue;
            ?>
            <li class="c-metric o-hero__metric">
    ...
    <?php if (!empty($art_id)): ?>
      <div class="o-hero__media" aria-hidden="true">
