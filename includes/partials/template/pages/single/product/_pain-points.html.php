  ...
  <div class="s-container">
    <?php if (!empty($title) || !empty($description)) : ?>
      <header class="o-product-pain-points__header">
        <?php if (!empty($title)) : ?>
          <h2 class="title__normal"><?php echo esc_html($title); ?></h2>
        <?php endif; ?>
        <?php if (!empty($description)) : ?>
          <div class="text__normal"><?php echo wpautop(wp_kses_post($description)); ?></div>
        <?php endif; ?>
      </header>
    <?php endif; ?>
    ...
    <?php if (!empty($cards)) : ?>
      <div class="o-product-pain-points__grid">
        ...
        ?>
          <article class="o-product-card o-product-pain-points__card">
            <?php if (!empty($icon)) : ?>
              <div class="o-product-card__icon"><?php echo wp_get_attachment_image($icon, 'thumbnail', false, array('loading' => 'lazy')); ?></div>
            <?php endif; ?>
